...
if raw_response['code'] in (200, 201):
	...
	for item in devices:
		...
		log_msg(f"\nProcessing data. (Extracting device properties from LM data to use in CW fields)", "DEBUG")
		all_properties = {}
...
for key, value in device_array.items():
	...
	if(key not in cw_devices):
		...
		if(patch_response['code'] == 200 or patch_response['code'] == 201):
			...
			print(f'Unable to update record for \"{key}\" with response code {patch_response["code"]}: {patch_response["body"]}', "ERROR")
